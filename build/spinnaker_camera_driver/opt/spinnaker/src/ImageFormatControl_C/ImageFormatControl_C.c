...
spinError ConfigureCustomImageSettings(spinNodeMapHandle hNodeMap)
{
    ...
    if (IsReadable(hPixelFormat, "PixelFormat"))
    {
    ...
    if (IsReadable(hPixelFormatMono8, "PixelFormatMono8"))
    {
    ...
    if (IsWritable(hPixelFormat, "PixelFormat"))
    {
    ...
    // get min
    if (IsReadable(hOffsetX, "OffsetX"))
    {
        err = spinIntegerGetMin(hOffsetX, &offsetXMin);
        if (err != SPINNAKER_ERR_SUCCESS)
        {
            printf("Unable to get offset x. Aborting with error %d...\n\n", err);
            return err;
        }
    }
    else
    ...
    // set min
    if (IsWritable(hOffsetX, "OffsetX"))
    {
    ...
    // get min
    if (IsReadable(hOffsetY, "OffsetY"))
    {
        err = spinIntegerGetMin(hOffsetY, &offsetYMin);
        if (err != SPINNAKER_ERR_SUCCESS)
        {
            printf("Unable to get offset Y. Aborting with error %d...\n\n", err);
            return err;
        }
    }
    else
    ...
    // set min
    if (IsWritable(hOffsetY, "OffsetY"))
    {
